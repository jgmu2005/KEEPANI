...
// Filas de productos (compartidas por todos los emails).
$rows = '';
foreach ($drops as $d) {
...
foreach ($subs as $u) {
    ...
    $html = '<div style="font-family:system-ui,sans-serif;max-width:600px">'
        ...
        . ($more > 0 ? '<p style="margin:14px 0"><a href="' . htmlspecialchars($base) . '/liquidaciones" style="color:#0369a1">…y ' . $more . ' más — ver todas ↗</a></p>' : '<p style="margin:14px 0"><a href="' . htmlspecialchars($base) . '/liquidaciones" style="color:#0369a1">Ver todas las liquidaciones ↗</a></p>')
    ...
    $res = $mailer->send((string) $u['email'], '🔥 ' . count($drops) . ' liquidaciones nuevas en Walmart', $html);
    if ($res['ok']) { $emailed++; }
}

$repo->markAllNotified();

out(200, ['ok' => true, 'pending' => count($drops), 'subscribers' => count($subs), 'emailed' => $emailed, 'more' => $more]);
